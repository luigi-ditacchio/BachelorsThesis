...
BEGIN
	IF @dis_trig IS NULL THEN
	BEGIN
		IF inclusione_struttura(NEW.codice)=FALSE OR NEW.numero < 0 THEN
			CALL eccezione();
		END IF;
	END;
	END IF;
...
BEGIN
	IF @dis_trig IS NULL THEN
	BEGIN
		IF OLD.codice <> NEW.codice OR OLD.numero <> NEW.numero THEN
		BEGIN
			IF inclusione_struttura(NEW.codice)=FALSE OR NEW.numero < 0 THEN
				CALL eccezione();
			END IF;
...
BEGIN
	IF @dis_trig IS NULL THEN
	BEGIN
		IF NEW.numOggetti <> 0 OR NEW.numero < 0 THEN
			CALL eccezione();
		END IF;
	END;
	END IF;
...
BEGIN
	...
	BEGIN
		...
		ELSEIF OLD.numero <> NEW.numero OR OLD.piano <> NEW.piano OR OLD.numero <> NEW.numero THEN
		BEGIN
			IF OLD.numOggetti <> 0 OR NEW.numero < 0 THEN
				CALL eccezione();
			END IF;
		END;
		END IF;
