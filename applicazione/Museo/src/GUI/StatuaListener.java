...
public class StatuaListener implements ActionListener {
	
	
	public static final String TASTO_PRINCIPALE_STATUA = "PRINCIPALE STATUA";
	public static final String TASTO_STATUA = "STATUA";
	public static final String TASTO_PIU_STATUA = "PIU STATUA";
	public static final String TASTO_PIU_ARTISTA = "PIU ARTISTA";
	
	
	private JPanel pannello_sinistro = new JPanel();
	private JPanel pannello_sinistro_statue = new JPanel();
	...
	private String statua_selezionata;
	private String artista_selezionato;
	
	
	public StatuaListener(MainFrame frame) {
		this.frame = frame;
	}
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (action == TASTO_PRINCIPALE_STATUA) {
			
			frame.pannello_superiore.removeAll();
			
			pannello_sinistro.removeAll();
			pannello_sinistro_statue.removeAll();
			...
			//STATUE
			pannello_sinistro_statue.setLayout(new BoxLayout(pannello_sinistro_statue, BoxLayout.Y_AXIS));
			TitledBorder title_statue = BorderFactory.createTitledBorder("STATUE");
			pannello_sinistro_statue.setBorder(new CompoundBorder(title_statue, new EmptyBorder(5, 20, 10, 20)));
			...
			pannello_sinistro.add(pannello_sinistro_statue, BorderLayout.CENTER);
			...
			Collection<String> statue = StatuaController.getCodiceTutti();
			if (statue == null) return;
			Iterator<String> it = statue.iterator();
			while (it.hasNext()) {
				JButton button = new JButton(String.valueOf(it.next()));
				button.addActionListener(frame.statua_listener);
				button.setActionCommand(StatuaListener.TASTO_STATUA);
				pannello_sinistro_statue.add(button);
			}
			...
			statua_selezionata = button.getText();
			
			JTextArea text_area = new JTextArea(StatuaController.getStatuaGenerali(statua_selezionata));
			text_area.setEditable(false);
			JScrollPane scroll = new JScrollPane(text_area);
			pannello_centrale_generali.add(scroll);
			
			JButton butt_par = new JButton("Piu' informazioni");
			butt_par.addActionListener(frame.statua_listener);
			butt_par.setActionCommand(StatuaListener.TASTO_PIU_STATUA);
			pannello_centrale_particolari.add(butt_par);
			
			
			artista_selezionato = StatuaController.getCodiceArtista(statua_selezionata);
			...
			butt_par_artista.addActionListener(frame.statua_listener);
			butt_par_artista.setActionCommand(StatuaListener.TASTO_PIU_ARTISTA);
			pannello_destro_particolari.add(butt_par_artista);
			
			
			JTextArea text_area_sala = new JTextArea(StatuaController.getCollocazione(statua_selezionata));
		...
		else if (action == TASTO_PIU_STATUA) {
			
			pannello_centrale_particolari.removeAll();

			JTextArea text_area = new JTextArea(StatuaController.getStatuaParticolari(statua_selezionata));
			text_area.setEditable(false);
			JScrollPane scroll = new JScrollPane(text_area);
